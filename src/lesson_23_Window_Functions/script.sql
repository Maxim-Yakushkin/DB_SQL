...
SELECT company.name,
       e.last_name,
--        count(e.id) OVER () count,
--        max(e.id) OVER () max_id,
--        row_number() OVER (),
       dense_rank() OVER (PARTITION BY company.name ORDER BY e.first_name) -- сортирует фамилии в алфавитном порядке в окне (по названию компании)
FROM company
